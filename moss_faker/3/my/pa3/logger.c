...
void log_started(local_id l_id, local_id to_id, balance_t s_balance) {
  ...
  sprintf(buf, log_started_fmt, get_lamport_time(), l_id, getpid(), getppid(), s_balance);
  printf("%s", buf);
...
void log_received_all_started(local_id l_id, local_id to_id, balance_t s_balance){
  char buf[MAX_MESSAGE_LEN];
  sprintf(buf, log_received_all_started_fmt, get_lamport_time(), l_id);
  printf("%s", buf);
  write(eventsLog, buf, strlen(buf));
}
...
void log_done(local_id l_id, local_id to_id, balance_t s_balance){
  char buf[MAX_MESSAGE_LEN];
  sprintf(buf, log_done_fmt, get_lamport_time(), l_id, s_balance);
  printf("%s", buf);
  write(eventsLog, buf, strlen(buf));
}
...
void log_transfer_out(local_id l_id, local_id to_id, balance_t s_balance){
  char buf[MAX_MESSAGE_LEN];
  sprintf(buf, log_transfer_out_fmt, get_lamport_time(), l_id, s_balance, to_id);
  printf("%s", buf);
  write(eventsLog, buf, strlen(buf));
}
...
void log_transfer_in(local_id l_id, local_id to_id, balance_t s_balance){
  char buf[MAX_MESSAGE_LEN];
  sprintf(buf, log_transfer_in_fmt, get_lamport_time(), l_id, s_balance, to_id);
  printf("%s", buf);
  write(eventsLog, buf, strlen(buf));
}
...
void log_received_all_done(local_id l_id, local_id to_id, balance_t s_balance){
  char buf[MAX_MESSAGE_LEN];
  sprintf(buf, log_received_all_done_fmt, get_lamport_time(), l_id);
  printf("%s", buf);
  write(eventsLog, buf, strlen(buf));
}
