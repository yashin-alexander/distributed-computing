...
static timestamp_t get_time() { return get_lamport_time(); }
...
void log_started(local_id l_id, local_id to_id, balance_t s_balance) {
  LOG_ACTION(eventsLog, log_started_fmt, get_time(), l_id, getpid(), getppid(), s_balance);
}

void log_received_all_started(local_id l_id, local_id to_id, balance_t s_balance){
  LOG_ACTION(eventsLog, log_received_all_started_fmt, get_time(), l_id);
}

void log_done(local_id l_id, local_id to_id, balance_t s_balance){
  LOG_ACTION(eventsLog, log_done_fmt, get_time(), l_id, s_balance);
}

void log_transfer_out(local_id l_id, local_id to_id, balance_t s_balance){
  LOG_ACTION(eventsLog, log_transfer_out_fmt, get_time(), l_id, s_balance, to_id);
}

void log_transfer_in(local_id l_id, local_id to_id, balance_t s_balance){
  LOG_ACTION(eventsLog, log_transfer_in_fmt, get_time(), l_id, s_balance, to_id);
}

void log_received_all_done(local_id l_id, local_id to_id, balance_t s_balance){
  LOG_ACTION(eventsLog, log_received_all_done_fmt, get_time(), l_id);
}
